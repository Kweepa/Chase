...
CHR_HELICOPTER = 2
CHR_TANK = 2
CHR_BONUS = 2  # playfield copy slots (heli or tank drawn here)
CHR_ENEMY = 18
CHR_BOLT = 6
CHR_EXPLOSION = 9
...
# Intact heli/tank: −2 vs Spectrum DE ($631A / $6C07). Exploded pair already at $6328.
ROM_HELI = 0x6318
ROM_HELI_END = 0x6328
ROM_BONUS_EXPLODED = 0x6328
ROM_BONUS_EXPLODED_END = 0x6338
ROM_TANK = 0x6C05
ROM_TANK_END = 0x6C15


def bonus_source_udgs(code: bytes) -> dict[str, list[bytes]]:
    """Heli/tank intact + shared exploded pair (copied into chr_bonus at runtime)."""
    heli = aligned_rows(code, ROM_HELI, ROM_HELI_END)
    tank = aligned_rows(code, ROM_TANK, ROM_TANK_END)
    exploded = aligned_rows(code, ROM_BONUS_EXPLODED, ROM_BONUS_EXPLODED_END)
    if len(heli) != CHR_HELICOPTER:
        raise ValueError(f"expected {CHR_HELICOPTER} heli rows, got {len(heli)}")
    if len(tank) != CHR_TANK:
        raise ValueError(f"expected {CHR_TANK} tank rows, got {len(tank)}")
    if len(exploded) != CHR_BONUS:
        raise ValueError(f"expected {CHR_BONUS} bonus-explode rows, got {len(exploded)}")
    return {
        "heli": heli,
        "tank": tank,
        "exploded": exploded,
...
def main() -> int:
    ...
    # Playfield copy target for the active bonus enemy (heli or tank).
    bonus_start = pool.reserve(CHR_BONUS, "bonus")
    meta["sprites"]["bonus"] = {"start": bonus_start, "count": CHR_BONUS}
    ...
    # Bonus enemy source UDGs (after playfield / handlebar slots; copy into chr_bonus).
    bonus_src = bonus_source_udgs(code)
    heli_start = len(pool.rows)
    for i, r in enumerate(bonus_src["heli"]):
        pool.add(r, "heli" if i == 0 else f"heli {i}")
    tank_start = len(pool.rows)
    for i, r in enumerate(bonus_src["tank"]):
        pool.add(r, "tank" if i == 0 else f"tank {i}")
    explode_start = len(pool.rows)
    for i, r in enumerate(bonus_src["exploded"]):
        pool.add(r, "bonus explode" if i == 0 else f"bonus explode {i}")
    meta["sprites"]["heli"] = {"start": heli_start, "count": CHR_HELICOPTER}
    meta["sprites"]["tank"] = {"start": tank_start, "count": CHR_TANK}
    meta["sprites"]["bonus_explode"] = {"start": explode_start, "count": CHR_BONUS}

    meta["playfield_chr"] = playfield_end
    meta["total_chr"] = len(pool.rows)
    over = playfield_end - PLAYFIELD_CHR_LIMIT
    meta["spare"] = -over if over > 0 else PLAYFIELD_CHR_LIMIT - playfield_end

    OUT_ASM.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "; Auto-generated by tools/convert_gfx.py — do not edit",
        f"; {len(pool.rows)} glyphs ({playfield_end} playfield"
        + (
        ...
        "gfx_playfield_size = " + str(playfield_end),
        "",
        "gfx_pool",
    ]
    ...
    equates.append(f"chr_heli = {meta['sprites']['heli']['start']}")
    equates.append(f"chr_tank = {meta['sprites']['tank']['start']}")
    equates.append(f"chr_bonus_explode = {meta['sprites']['bonus_explode']['start']}")
    equates.append("")
    ...
    spare_msg = (
        f"{meta['spare']} spare playfield in {PLAYFIELD_CHR_LIMIT}"
        if meta["spare"] >= 0
        else f"{-meta['spare']} over {PLAYFIELD_CHR_LIMIT} playfield"
    )
    ...
    print(
        f"  bonus: playfield chr {meta['sprites']['bonus']['start']}..+{CHR_BONUS}, "
        f"heli {heli_start}, tank {tank_start}, explode {explode_start}"
    )
